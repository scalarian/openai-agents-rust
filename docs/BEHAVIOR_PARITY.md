...
| `test_openai_chatcompletions_stream` | `omitted-with-rationale` | `n/a` | Rust has helper-level `chatcmpl_stream_handler` coverage for single-chunk delta parsing, but the broader upstream streaming family still lacks executable end-to-end streaming coverage for event ordering and final assembly; that helper-level test is not sufficient parity evidence. |
