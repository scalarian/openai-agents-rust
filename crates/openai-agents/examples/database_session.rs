...
struct DatabaseSessionModel;

#[async_trait]
impl Model for DatabaseSessionModel {
    async fn generate(&self, request: ModelRequest) -> AgentsResult<ModelResponse> {
        ...
        let text = if history.contains("population") {
            ...
            "I can answer from database-backed session context.".to_owned()
        };
...
#[derive(Clone, Default)]
struct DatabaseSessionProvider {
    model: Arc<DatabaseSessionModel>,
}

impl ModelProvider for DatabaseSessionProvider {
...
async fn main() -> Result<(), AgentsError> {
    ...
    let runner = Runner::new().with_model_provider(Arc::new(DatabaseSessionProvider::default()));

    println!("=== Database Session Example ===");
    println!("connection_string={}", session.connection_string);
