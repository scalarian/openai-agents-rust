...
cargo run -p openai-agents-rs --example dapr_session
...
cargo run -p openai-agents-rs --example image_generator
...
cargo run -p openai-agents-rs --example mcp_tool_filter
...
| `dapr_session` | Dapr state-store backed extension session with graceful availability check | [docs/sessions/memory.md](../docs/sessions/memory.md) |
...
| `image_generator` | hosted image generation tool configuration and result decoding | [docs/tools.md](../docs/tools.md) |
...
| `mcp_tool_filter` | static MCP tool filtering before model-visible tool discovery | [docs/mcp.md](../docs/mcp.md) |
